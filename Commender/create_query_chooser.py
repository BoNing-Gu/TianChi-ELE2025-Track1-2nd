...
def messages_builder(row):
    with open(f'src/system_prompt_chooser.txt', 'r', encoding='utf-8') as f:
    ...
    用户点餐需求："{text}"
    菜品列表："{food_pool}"
    你觉得哪种菜品最能接受？请根据用户需求进行判断和打分，然后按照打分从高到低对菜品进行排序，最终以JSON格式返回得分最高的菜品字段（包含菜品名称、评分和理由），不超过五个菜品。
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", "-n", 
                        type=str, 
                        choices=["slover-ds+cluster-1.1_1.2+searcher-bge", 
