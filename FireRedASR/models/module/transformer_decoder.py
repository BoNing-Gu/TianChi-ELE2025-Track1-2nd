...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt_seq, encoder_output, encoder_mask, cache=None):
        """
        Args:
            tgt_seq: (batch_size, tgt_len)
            encoder_output: (batch_size, src_len, d_model)
            encoder_mask: (batch_size, 1, src_len)
            cache: List[Optional[Tensor]] for incremental decoding
        Returns:
            dec_output: (batch_size, tgt_len, d_model)
        """
        # Prepare masks
        tgt_mask = self.ignored_target_position_is_0(tgt_seq, self.pad_id)
        
        # Word embedding + positional encoding
        dec_output = self.dropout(
            self.tgt_word_emb(tgt_seq) * self.scale + 
            self.positional_encoding(tgt_seq))
